...
test("LogoStrip renders without heading", async () => {
  const container = await AstroContainer.create();
  const html = await container.renderToString(LogoStrip, {
    props: {
      logos: [{ src: "assets/logo-a.svg", alt: "Partner A" }],
    },
  });
  expect(html).toContain("assets/logo-a.svg");
  expect(html).not.toContain("logo-strip__heading");
});
