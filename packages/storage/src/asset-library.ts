...
export interface Asset {
  ...
  /** Path relative to businessDir, e.g. "library/ast_x.webp". */
  file: string;
  mime: string;
  ...
  tags: AssetTags;
  altText?: string;
  /** Photo credit string (GMB author attributions, photographer name, etc.). */
  attribution?: string;
  /** SHA-256 hex of the raw file bytes — content-addressed deduplication. */
  contentHash?: string;
  /** Stable source reference for pre-download dedup, e.g. a GMB resource name. */
  sourceRef?: string;
  usages: AssetUsage[];
  status: "active" | "archived";
  createdAt: string;
  /** Which site this asset originated from. Undefined = generic (equipment, AI, etc.). */
  siteOrigin?: string;
}
...
/** Find an active asset by SHA-256 content hash. */
export function findByHash(library: AssetLibrary, hash: string): Asset | undefined {
  return Object.values(library.assets).find((a) => a.contentHash === hash && a.status === "active");
}

/** Find an active asset by stable source reference (e.g. GMB resource name). */
export function findBySourceRef(library: AssetLibrary, ref: string): Asset | undefined {
  return Object.values(library.assets).find((a) => a.sourceRef === ref && a.status === "active");
}
