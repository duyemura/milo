/**
 * capture — consolidated hardened engine. Deterministic DOM→computed-style
 * transcription:
 *
 ...
 *
 * Ported byte-faithfully from page-clone-spike/page-clone.mjs — same algorithm,
 * same output. Only mechanical changes: CLI args → opts, top-level IIFE body →
 * function, self-containment `process.exit(1)` → `throw` (a library must not kill
 * the host process; a thrown error still yields a non-zero CLI exit), and the
 * browser is closed in `finally` so it always closes on error (output-neutral).
 */
...
export async function capture(opts: CaptureOpts): Promise<{ capture: CaptureJson; outDir: string }> {
  ...
  async function neutralizeAndTag(keepAttrs: string[]) {
    ...
    for (const el of document.querySelectorAll("*")) {
      ...
      if (parseFloat(cs.opacity) < 0.99) {
        ...
        if ((leaf && (el as HTMLElement).textContent!.trim()) || el.querySelector("img,svg,picture,video")) (el as HTMLElement).style.setProperty("opacity", "1", "important");
      }
    }
    ...
    function walk(el: Element): any {
      ...
      const node: any = { id: n++, tag: el.tagName.toLowerCase(), attrs: {}, children: [] };
      ...
      if (el.tagName === "IMG") { node.attrs.src = (el as HTMLImageElement).currentSrc || (el as HTMLImageElement).src; }
      else if ((el as any).src) node.attrs.src = (el as any).src;
      if (el.tagName === "A" && (el as HTMLAnchorElement).href) node.attrs.href = (el as HTMLAnchorElement).href;
      ...
      for (const cn of el.childNodes) {
        ...
        } else if (cn.nodeType === 1) { const c = walk(cn as Element); if (c) node.children.push(c); }
      }
      return node;
    }
  ...
  function grabHead() {
    ...
    return {
      ...
      fontFaces: (() => { let css = ""; for (const s of document.styleSheets) { let r: any; try { r = s.cssRules; } catch { continue; } for (const rule of r) if (rule.constructor.name === "CSSFontFaceRule") css += rule.cssText + "\n"; } return css; })(),
    };
  }

  // ---------- node: asset collection / rehost ----------
  const URL_RE = /url\((['"]?)([^'")]+)\1\)/g;
  const absolutize = (u: string): string | null => { try { return new URL(u, SRC_URL).href; } catch { return null; } };

  const NO_REHOST = new Set(["iframe", "embed", "object"]); // live embeds (maps/widgets) — keep their real src, never rehost
  function collectFromTree(node: TreeNode, set: Set<string>): Set<string> {
    if ((node as any).t !== undefined) return set;
    const el = node as TreeEl;
    if (el.attrs.src && !NO_REHOST.has(el.tag)) { const a = absolutize(el.attrs.src); if (a) set.add(a); }
    if (el.attrs.poster) { const a = absolutize(el.attrs.poster); if (a) set.add(a); }
    if (el.attrs.srcset) for (const part of el.attrs.srcset.split(",")) { const a = absolutize(part.trim().split(/\s+/)[0]); if (a) set.add(a); }
    el.children.forEach((c) => collectFromTree(c, set));
    return set;
  }
  function collectFromStyles(styleMaps: StylesByWidth, set: Set<string>): Set<string> {
  ...
  function sniffExt(b: Buffer): string | null { // type by magic bytes; "HTML" means it's a document, not a static asset
  ...
  function rewriteTree(node: TreeNode, map: Map<string, string>) {
    if ((node as any).t !== undefined) return;
    const el = node as TreeEl;
    const sub = (u: string) => map.get(absolutize(u)!) || u;
    if (el.attrs.src && !NO_REHOST.has(el.tag)) el.attrs.src = sub(el.attrs.src);
    if (el.attrs.poster) el.attrs.poster = sub(el.attrs.poster);
    if (el.attrs.srcset) el.attrs.srcset = el.attrs.srcset.split(",").map((s) => { const [u, d] = s.trim().split(/\s+/); return sub(u) + (d ? " " + d : ""); }).join(", ");
    el.children.forEach((c) => rewriteTree(c, map));
  }
  ...
  function render(node: TreeNode): string {
    ...
    const el = node as TreeEl;
    let a = ` class="pc-${el.id}"`;
    for (const [k, v] of Object.entries(el.attrs)) a += ` ${k}="${escA(v)}"`;
  ...
  try {
    ...
    // ---- rehost ----
    const assetSet = collectFromStyles(styles, collectFromTree(tree, new Set<string>()));
    ...
    const flag = (u: string) => { try { if (!u) return; const t = u.startsWith("//") ? "https:" + u : u; if (/^https?:/.test(t) && sourceOrigins.has(new URL(t).host)) leftovers.push(u); } catch { /* unparseable fragment (e.g. comma inside a srcset URL) — ignore, don't throw */ } };
    const scanTree = (node: TreeNode) => { if ((node as any).t !== undefined) return; const el = node as TreeEl; for (const key of ["src", "srcset", "poster"]) if (el.attrs[key]) for (const part of String(el.attrs[key]).split(",")) flag(part.trim().split(/\s+/)[0]); el.children.forEach(scanTree); };
    scanTree(tree);
    ...
    if (missing.length) { throw new Error(`SELF-CONTAINMENT FAILED: ${missing.length} rehosted asset file(s) missing on disk — not shipping a broken clone`); } // as a library we throw (non-zero CLI exit) instead of process.exit(1)
...
${css}</style></head><body class="pc-${tree.id}">${bodyInner}</body></html>`;
    ...
    const captureJson: CaptureJson = { tree, styles, head, fontCss, interactions, sourceOrigins: [...sourceOrigins] };
    fs.writeFileSync(path.join(OUT, "capture.json"), JSON.stringify(captureJson));
    ...
    if (!VERIFY) { return { capture: captureJson, outDir: OUT }; }
    const shoot = async (w: number, name: string) => {
    ...
    return { capture: captureJson, outDir: OUT };
  } finally {
    await browser.close();
  }
}
